    ...
    <div className="app-container">
      <header className="app-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          {!currentFile && (
             <span className="brand-title" style={{marginLeft: '8px'}}>Kumpulan Buah Pikir</span>
          )}
          {currentFile && (
             <span className="brand-title" style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>{currentFile.title}</span>
          )}
        </div>
        ...
        {currentFile ? (
          ...
          <div className="library-container">
            ...
            <div className="library-grid">
              {FILE_LIST.map(file => (
                <div key={file.id} className="library-card" onClick={() => setCurrentFile(file)}>
              ...
              {FILE_LIST.length === 0 && (
                <p style={{ opacity: 0.6 }}>Belum ada tulisan. Tambahkan file Markdown (.md) ke dalam folder <code>src/content/</code>.</p>
              )}
            </div>
          </div>
        )}
